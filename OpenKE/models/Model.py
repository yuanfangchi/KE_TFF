...
class Model(tf.keras.Model):
	...
	def get_all_instance(self, in_batch = False):
		if in_batch:
			return [tf.transpose(a=tf.reshape(self.batch_h, [1 + self.config.negative_ent + self.config.negative_rel, -1]), perm=[1, 0]),\
			tf.transpose(a=tf.reshape(self.batch_t, [1 + self.config.negative_ent + self.config.negative_rel, -1]), perm=[1, 0]),\
			tf.transpose(a=tf.reshape(self.batch_r, [1 + self.config.negative_ent + self.config.negative_rel, -1]), perm=[1, 0])]
		else:
			return [self.batch_h, self.batch_t, self.batch_r]

	def get_all_labels(self, in_batch = False):
		if in_batch:
			return tf.transpose(a=tf.reshape(self.batch_y, [1 + self.config.negative_ent + self.config.negative_rel, -1]), perm=[1, 0])
		else:
			return self.batch_y
	...
	def input_def(self):
		...
		self.positive_h = tf.transpose(a=tf.reshape(self.batch_h[0:config.batch_size], [1, -1]), perm = [1, 0])
		self.positive_t = tf.transpose(a=tf.reshape(self.batch_t[0:config.batch_size], [1, -1]), perm = [1, 0])
		self.positive_r = tf.transpose(a=tf.reshape(self.batch_r[0:config.batch_size], [1, -1]), perm = [1, 0])
		self.positive_y = tf.transpose(a=tf.reshape(self.batch_y[0:config.batch_size], [1, -1]), perm = [1, 0])
		self.negative_h = tf.transpose(a=tf.reshape(self.batch_h[config.batch_size:config.batch_seq_size], [config.negative_ent + config.negative_rel, -1]), perm = [1, 0])
		self.negative_t = tf.transpose(a=tf.reshape(self.batch_t[config.batch_size:config.batch_seq_size], [config.negative_ent + config.negative_rel, -1]), perm = [1, 0])
		self.negative_r = tf.transpose(a=tf.reshape(self.batch_r[config.batch_size:config.batch_seq_size], [config.negative_ent + config.negative_rel, -1]), perm = [1, 0])
		self.negative_y = tf.transpose(a=tf.reshape(self.batch_y[config.batch_size:config.batch_seq_size], [config.negative_ent + config.negative_rel, -1]), perm = [1, 0])
