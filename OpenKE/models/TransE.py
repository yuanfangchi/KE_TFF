...
class TransE(Model):
	...
	def embedding_def(self):
		...
		self.ent_embeddings = tf.compat.v1.get_variable(name = "ent_embeddings", trainable=True, shape = [config.entTotal, config.hidden_size])
		self.rel_embeddings = tf.compat.v1.get_variable(name = "rel_embeddings", trainable=True, shape = [config.relTotal, config.hidden_size])
	...
	def predict_def(self):
		...
		self.predict = tf.reduce_mean(input_tensor=self._calc(predict_h_e, predict_t_e, predict_r_e), axis=1, keepdims = False)